...
void painter::draw (world* sim, float scale) {
  ...
  for (int i = 0; i < sim->intc; i++)
    draw(sim->intersections[i], scale);
  
  for (int i = 0; i < sim->roadc; i++) {
    draw(sim->roads[i], scale);
    drawLights(sim->roads[i], scale);

    for (int j = 0; i < sim->roads[i]->length; j++)
      {
	if (sim->roads[i]->cars[j])
	  draw(sim->roads[i]->cars[j], scale);
      }
  }
  
  // for (int i = 0; i < sim->carc; i++)
  //   sim->cars[i]->viewCar (sim->scale);
